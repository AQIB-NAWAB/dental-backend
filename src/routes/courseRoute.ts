...
router.get("/api/pending-users",currentUser,requireAuth,async(req:Request,res:Response)=>{


    const activeCourses=await Ticket.find({status:"pending"}).populate('createdBy').populate('packageId').populate('courseId');
